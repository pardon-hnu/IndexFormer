...
from model.time_ratio_dataset import PlanTreeDataset
from model.time_ratio_trainer import eval_workload, train,evaluate,predict
...
class Args:
    ...
    sch_decay = 0.6
    device = 'cuda:0'
    newpath = './results/ratio/best_checkpoints_tpcds/'
    to_predict = 'cost' 
    dataset = 'tpcds'
def tpcds_time_ratio_run(toe,pt_file):
    args = Args()
    ...
    for i in range(1,20):
        if i in {9,14,17,15}:
            continue
        file = 'data/tpcds/tpcds_timecompare_new/output{}_withidx1_timecompare.csv'.format(i)
        df = pd.read_csv(file)
        # 打乱当前 DataFrame

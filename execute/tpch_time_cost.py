...
class Args:
    ...
    sch_decay = 0.6
    device = 'cuda:0'
    newpath = './results/cost/best_checkpoints_tpch/'
    to_predict = 'cost'
    dataset = 'tpch'
def tpch_time_cost_run(toe,pt_file):
    args = Args()
    ...
    for i in range(1,20):
        if i in {9,14,17,15}:
            continue
        file = 'data/tpch/tpch10_idx/output{}_withidx1.csv'.format(i)
        df = pd.read_csv(file)
        # 按比例拆分为训练集和测试集
    ...
    for i in range(1,20):
        if i in {9,14,17,15}:
            continue
        file = 'data/tpch/tpch10_query_plan_new/output{}.csv'.format(i)
        df = pd.read_csv(file)
        # 打乱当前 DataFrame
        train_size = int(0.9 * len(df))
        full_train_df = full_train_df.append(df.iloc[:train_size], ignore_index=True)  # 前90%加入训练集

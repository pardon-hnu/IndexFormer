...
class Args:
    ...
    sch_decay = 0.6
    device = 'cuda:0'
    newpath = './results/ratio/best_checkpoints_imdb/'
    to_predict = 'cost' 
    dataset = 'imdb'
def imdb_time_ratio_run(toe,pt_file):
    args = Args()
    ...
    for i in range(1,20):
        if i in {9,14,17,15}:
            continue
        file = 'data/imdb10/imdb_timecompare_new/output{}_withidx1_timecompare.csv'.format(i)
        df = pd.read_csv(file)
        # 打乱当前 DataFrame

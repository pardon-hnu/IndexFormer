...
class Args:
    ...
    sch_decay = 0.6
    device = 'cuda:0'
    newpath = './results/cost/best_checkpoints_tpcds/'
    to_predict = 'cost'
    dataset = 'tpcds'
def tpcds_time_cost_run(toe,pt_file):
    args = Args()
    ...
    for i in range(1,100):
        if i in {14,24,41,44,53,72,78,86}:
            continue
        file = 'data/tpcds/tpcds_idx/output{}_withidx1.csv'.format(i)
        df = pd.read_csv(file)
    ...
    for i in range(1,100):
        if i in {14,24,41,44,53,72,78,86}:
            continue
        file = 'data/tpcds/tpcds_query_plan/output{}.csv'.format(i)
        df = pd.read_csv(file)
